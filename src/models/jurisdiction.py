...
class JurisdictionMetadata(BaseModel):
    ...
    urls: list[URLObject] = Field(
        ...,
        description="List of URLs related to the jurisdiction; additional arbitrary key-value pairs allowed on this model.",
    )
...
class SessionDetail(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of session, typically a year span like 2011-2012. **(required)**",
    )
...
class TermDetail(BaseModel):
    ...
    term_description: str = Field(
        ...,
        description="The legal language defining how the terms is proscribed in the source document. i.e Shall commence the second tuesday following the last general election and any other useful information.",
    )
...
if __name__ == "__main__":
    sample = Jurisdiction(
        ...
        feature_flags=[
            {"legislative_sessions": -1, "term_detail": 1}
        ],  # Where "1" means expect this feature and "-1" means not applicable.
    )
