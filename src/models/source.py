...
from pydantic import AnyHttpUrl, BaseModel, Field, FileUrl, FtpUrl
...
class SourceObj(BaseModel):
    ...
    field: list[str] = Field(
        description="An array of fields that this SourceObj refers to, can be more than one. Validated against the available fields in the model."
    )
    ...
    source_url: dict[str, AnyHttpUrl | FtpUrl | FileUrl] = Field(
